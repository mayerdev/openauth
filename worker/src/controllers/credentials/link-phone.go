...
type LinkPhoneStartRequest struct {
...
type LinkPhoneConfirmRequest struct {
	AccessToken           string `json:"access_token"`
	VerificationSessionID string `json:"verification_session_id"`
	Code                  string `json:"code"`
}

func LinkPhoneStart(msg *nats.Msg) {
	var req LinkPhoneStartRequest
	...
	if req.Phone == "" {
		msg.Respond(types.EmitError("Phone is required", types.NoErrors))
		return
	}
	...
	user, _, err := credentials.FindUserByCredential(utils.Database, credentials.CredentialTypePhone, phone)
	...
	if user != nil {
		if user.ID == claims.UserID {
			msg.Respond(types.EmitError("Phone already linked", types.NoErrors))
		} else {
			msg.Respond(types.EmitError("Phone already in use", types.NoErrors))
		}
	...
	sessionID, err := sessions.CreateVerificationSession(ctx, claims.UserID, credentials.CredentialTypePhone, phone, codeTTL, nil)
	if err != nil {
		msg.Respond(types.EmitError("Internal error", types.NoErrors))
		return
	}
	...
	if err := sessions.StoreVerificationCode(ctx, sessionID, claims.UserID, code, credentials.CredentialTypePhone, codeTTL); err != nil {
		msg.Respond(types.EmitError("Internal error", types.NoErrors))
		return
	}
...
func LinkPhoneConfirm(msg *nats.Msg) {
	var req LinkPhoneConfirmRequest
	if err := json.Unmarshal(msg.Data, &req); err != nil {
		msg.Respond(types.EmitError("Invalid request body", types.NoErrors))
		return
	}
	...
	if err := utils.Database.Transaction(func(tx *gorm.DB) error {
		...
		if err.Error() == "phone already in use" {
			msg.Respond(types.EmitError("Phone already in use", types.NoErrors))
		} else {
			msg.Respond(types.EmitError("Internal error", types.NoErrors))
		}
